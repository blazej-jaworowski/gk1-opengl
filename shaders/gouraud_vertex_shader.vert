...
vec3 get_dir_light(vec3 norm) {
    vec3 frag_pos = vec3(model_matrix * vec4(aPos, 1.0)).xyz;
    vec3 sum = vec3(0);
...
vec3 apply_fog(vec3 color, float ls, float le, vec3 fog_color, float dist) {
...
void main()
{
    ...
    vec3 norm = normalize(mat3(transpose(inverse(model_matrix))) * aNormal);
    color = get_dir_light(norm) + get_spot_light(norm);
    if(fog_enabled) {
        float dist = gl_Position.z;
        color = apply_fog(color, 2, fog_dist, fog_color, dist);
    }
}